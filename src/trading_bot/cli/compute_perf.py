...
def get_initial_stop_for_pair(pair: dict, open_positions_by_symbol: dict) -> float:
    """Resolve the initial stop price used to risk-size a closed pair, in
    priority order:
      1. A matching entry in open_positions.json (best-effort - typically
         won't match, since a closed position has already been removed
         from that file by the time this runs).
      2. A stop_price column in trades.csv, if present (not emitted by the
         current trade.py, but reserved for future use).
      3. Fallback proxy: assume the bot's own default 1% stop distance
         under entry (consistent with cycle.py's entry*0.99 convention),
         i.e. initial_stop = buy_price * 0.99.
    """
    symbol = pair["symbol"]
    ...
    for pos in open_positions_by_symbol.get(symbol, []):
        ...
        if entry is not None and initial_stop is not None and abs(float(entry) - buy_price) < 0.01:
            return float(initial_stop)

    # trades.csv has no stop_price column in the current schema - reserved
    # for a future extension; nothing to look up here yet.

    return buy_price * 0.99


def compute_r_for_pairs(closed_pairs: list[dict], open_positions: list[dict]) -> list[dict]:
    """Return closed_pairs with an added 'r_multiple' field (float or None
    if the risk distance is zero/undefined)."""
    ...
    for pair in closed_pairs:
        pair = dict(pair)
        initial_stop = get_initial_stop_for_pair(pair, open_positions_by_symbol)
        risk_per_share = pair["buy_price"] - initial_stop
        if risk_per_share == 0:
...
def main() -> None:
    ...
    try:
        ...
        pairs_with_r = compute_r_for_pairs(closed_pairs, open_positions)
