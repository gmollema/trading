"""Test different entry RSI levels on RSI2 strategy."""
...
def test_symbol(symbol: str, label: str, optimal_stop: float, optimal_sma: int):
    ...
    print(f"\n{'='*80}")
    print(f"Testing different entry RSI levels on {label} ({symbol})")
    print(f"Post-video period (2021-07-01 onwards)\n")
    print(f"{'Entry RSI':<12} {'Trades':<10} {'Net Pts':<12} {'Win %':<10} {'Avg/Trade':<12} {'Max DD':<10}")
    print("-" * 80)

    results = []

    for entry_rsi in [5, 7, 10, 15, 20]:
        trades = find_rsi2_long_trades(
            bars,
            rsi_period=2,
            entry_level=float(entry_rsi),
            exit_level=70.0,
        ...
        marker = " <-- BASELINE" if entry_rsi == 10 else ""
        print(f"{entry_rsi:<12} {summary['trades']:<10} {summary['net_points']:<12.1f} {summary['win_pct']:<10.1f} {summary['avg_points']:<12.1f} {summary['max_drawdown_points']:<10.1f}{marker}")
    ...
    if results:
        ...
        baseline = [r for r in results if r['entry_rsi'] == 10][0]
        improvement = best['net'] - baseline['net']
        improvement_pct = (improvement / abs(baseline['net'])) * 100 if baseline['net'] != 0 else 0

        print(f"\nOPTIMAL: RSI({best['entry_rsi']}) -> {best['net']:.1f} points ({best['trades']} trades)")
        if improvement > 0:
            print(f"vs baseline (RSI 10): +{improvement:.1f} points (+{improvement_pct:.1f}%)")
        else:
            print(f"vs baseline (RSI 10): {improvement:.1f} points ({improvement_pct:.1f}%)")
